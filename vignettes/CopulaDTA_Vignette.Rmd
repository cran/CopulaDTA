...
```
The code for the fitted model is displayed as follows
...
From the output above, `n_eff` and `Rhat` both confirm proper mixing of the chains with little autocorrelation. The meta-analytic sensitivity `MUse[1]` and specificity `MUsp[1]` is `0.7540 [0.6460, 0.8119]` and `0.8006 [0.6235, 0.9053]` respectively. The Kendall's tau correlation between sensitivity and specificity is estimated to be `-0.8436 [-0.9772, -0.3394]`. 
...
```
The Clayton copula is known to be unstable when the correlation parameter is close to the boundaries (-1 or 0) and this could be the reason why sampling from the posterior distribution was difficult.
